...
FutureOr<String?> _getRouteLevelRedirect(
  ...
  FutureOr<String?> processRouteRedirect(String? newLocation) =>
      newLocation ?? _getRouteLevelRedirect(context, configuration, matchList, currentCheckIndex + 1);
  final RouteBase route = match.route;
  FutureOr<String?> routeRedirectResult;
  if (route is Route && route.redirect != null) {
...
RouteMatchList _getNewMatches(
  ...
  try {
    ...
    _addRedirect(redirectHistory, newMatch, previousLocation, configuration.redirectLimit);
    return newMatch;
...
RouteMatchList _handleRedirectionError(RedirectionError error) {
  log('Redirection error: ${error.message}');
...
void _addRedirect(List<RouteMatchList> redirects, RouteMatchList newMatch, Uri prevLocation, int redirectLimit) {
