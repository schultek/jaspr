...
abstract class IslandComponent extends StatelessComponent {
  ...
  Iterable<Component> build(BuildContext context) sync* {
    var app = context.dependOnInheritedComponentOfExactType<_InheritedIslandsApp>();

    if (app == null) {
      throw AssertionError(
          'Missing IslandsApp in component tree. When using islands, make sure to insert the IslandsApp component at '
          'the root of your widget tree.');
    }

    var id = app.state.registerIsland(name, params);
...
class _IslandsAppState extends State<IslandsApp> with SyncStateMixin<IslandsApp, Map<String, String>> {
  ...
  @override
  Iterable<Component> build(BuildContext context) sync* {
    yield _InheritedIslandsApp(state: this, child: component.child);
  }
}

class _InheritedIslandsApp extends InheritedComponent {
  const _InheritedIslandsApp({required this.state, required super.child});

  final _IslandsAppState state;

  @override
  bool updateShouldNotify(covariant _InheritedIslandsApp oldComponent) {
