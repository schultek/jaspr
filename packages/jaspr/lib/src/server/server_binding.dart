...
class ServerAppBinding extends AppBinding with ComponentsBinding {
  ...
  Map<Element, Diagnosticable> _adaptersToDiagnosticableMap(List<RenderAdapter> adapters) {
    final map = <Element, Diagnosticable>{};
    for (final adapter in adapters) {
      if (adapter is ElementBoundaryAdapter) {
        map[adapter.element] = adapter;
      }
    }
    return map;
  }
