...
class _StreamBuilderBaseState<T, S> extends State<StreamBuilderBase<T, S>> {
  ...
  @override
  void didUpdateComponent(StreamBuilderBase<T, S> oldComponent) {
    super.didUpdateComponent(oldComponent);
    if (oldComponent.stream != component.stream) {
      if (_subscription != null) {
        _unsubscribe();
        _summary = component.afterDisconnected(_summary);
      }
      _subscribe();
    }
  }
...
class AsyncSnapshot<T> {
  ...
  /// set to an initial data value specified by the relevant component. See
...
/// Signature for strategies that build components based on asynchronous
/// interaction.
///
/// See also:
///
///  * [StreamBuilder], which delegates to an [AsyncComponentBuilder] to build
...
///    [Stream].
///  * [StreamBuilderBase], which supports component building based on a computation
...
class StreamBuilder<T> extends StreamBuilderBase<T, AsyncSnapshot<T>> {
  ...
  ///
  /// This builder must only return a component and should not have any side
  /// effects as it may be called multiple times.
  final AsyncComponentBuilder<T> builder;
...
/// [State.didUpdateComponent], or [State.didChangeDependencies]. It must not be
/// created during the [State.build] or [StatelessComponent.build] method call when
...
class _FutureBuilderState<T> extends State<FutureBuilder<T>> {
  ...
  /// or after component reconfiguration to a new Future.
  ...
  @override
  void didUpdateComponent(FutureBuilder<T> oldComponent) {
    super.didUpdateComponent(oldComponent);
