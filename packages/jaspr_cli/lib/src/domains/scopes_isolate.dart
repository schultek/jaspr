...
sealed class ScopesIsolateMessage {}

class InitializeMessage extends ScopesIsolateMessage {
  final List<String> entryPaths;
  final bool allowServerLibsInClient;
  final String dartSdkPath;
  final SendPort sendPort;
  InitializeMessage(this.entryPaths, this.allowServerLibsInClient, this.dartSdkPath, this.sendPort);
}

class UpdateMessage extends ScopesIsolateMessage {
  final List<String> entryPaths;
  final bool allowServerLibsInClient;
  UpdateMessage(this.entryPaths, this.allowServerLibsInClient);
}

class DisposeMessage extends ScopesIsolateMessage {}

sealed class ScopesIsolateEvent {}

class StatusEvent extends ScopesIsolateEvent {
  final String contextRoot;
  final bool isAnalyzing;
  StatusEvent(this.contextRoot, this.isAnalyzing);
}

class ResultEvent extends ScopesIsolateEvent {
  final String contextRoot;
  final Map<String, SerializedInspectDataItem> libraries;
  ResultEvent(this.contextRoot, this.libraries);
}

class LogEvent extends ScopesIsolateEvent {
  final String message;
  final Level level;
  LogEvent(this.message, {this.level = Level.info});
}

class RecreateScopesEvent extends ScopesIsolateEvent {}

class ScopesIsolate {
  ScopesIsolate({required this.entryPaths, required this.allowServerLibsInClient, this.eventPort});
  ...
  static Future<void> spawn(SendPort sendPort) async {
    ...
    final isolate = ScopesIsolate(entryPaths: [], allowServerLibsInClient: false);
    isolate.setup(receivePort);
  }

  void setup(ReceivePort receivePort) {
    receivePort.listen((message) async {
      if (message is InitializeMessage) {
        ...
        dartSdkPath = message.dartSdkPath;
  ...
  Future<void> initialize() async {
    log('Initializing scopes for $entryPaths...');
      ...
      watcherSubscriptions.add(
        DirectoryWatcher(context.contextRoot.root.path).events.listen((event) {
          ...
          if (path.endsWith('.server.dart') && event.type != ChangeType.MODIFY) {
            eventPort?.send(RecreateScopesEvent());
    ...
    await [for (final context in collection!.contexts) analyze(context)].wait;
  }

  void log(String message, {Level level = Level.info}) {
    eventPort?.send(LogEvent(message, level: level));
  }
  ...
  Future<void> analyze(AnalysisContext context, [bool awaitPendingChanges = false]) async {
    ...
    try {
      if (awaitPendingChanges) {
        ...
        sw.stop();
        log('Applied pending changes in ${sw.elapsedMilliseconds}ms');
      }

      log('Analyzing $rootPath...');
      eventPort?.send(StatusEvent(rootPath, true));
      ...
      for (final result in results) {
        if (result is! ResolvedLibraryResult) {
          ...
          log('Failed to resolve "$target" in "$rootPath"');
          continue;
        }
        libraries.add(result.element);
      }

      log('Resolved ${libraries.length} libraries in "$rootPath" in ${sw.elapsedMilliseconds}ms');

      if (libraries.isEmpty) return;

      final inspectData = await InspectData.analyze(libraries, allowServerLibsInClient, log);
      final serialized = inspectData.libraries.map((k, v) => MapEntry(k, v.serialize()));

      eventPort?.send(ResultEvent(rootPath, serialized));
    } on InconsistentAnalysisException catch (_) {
      log('Skipping inconsistent analysis for $rootPath');
    } catch (e) {
      log('Error analyzing $rootPath: $e');
    } finally {
      eventPort?.send(StatusEvent(rootPath, false));
    }
  }

  void reanalyze(String path) {
  ...
  Future<void> dispose() async {
    ...
    for (final sub in watcherSubscriptions) {
      sub.cancel();
    }
...
class InspectData {
  InspectData._(this.allowServerLibsInClient, this.log);
  static Future<InspectData> analyze(
    List<LibraryElement> libraries,
    bool allowServerLibsInClient,
    void Function(String message, {Level level}) log,
  ) async {
    final inspectData = InspectData._(allowServerLibsInClient, log);
  ...
  final bool allowServerLibsInClient;
  final void Function(String message, {Level level}) log;
...
class InspectDataItem {
  ...
  SerializedInspectDataItem serialize() {
    return SerializedInspectDataItem(
      uri: library.uri.toString(),
      components: components,
      clientScopeRoots: clientScopeRoots.toList(),
      serverScopeRoots: serverScopeRoots.toList(),
      children: children.map((c) => c.serialize()).toList(),
    );
  }
  ...
  Future<List<({LibraryElement lib, DirectiveTarget dir, bool onClient, bool onServer})>> resolveDependencies(
    ...
    if (result is! ParsedLibraryResult) {
      data.log('Tooling Daemon: Failed to parse library ${library.uri}', level: Level.warning);
      return [];
    }
...
class InspectItemDependency {
  ...
  SerializedInspectItemDependency serialize() {
    return SerializedInspectItemDependency(
      uri: item.library.uri.toString(),
      dir: dir,
      onClient: onClient,
      onServer: onServer,
      invalidOnClient: invalidOnClient,
      invalidOnServer: invalidOnServer,
    );
  }
}

class SerializedInspectDataItem {
  final String uri;
  final List<InspectTarget> components;
  final List<InspectTarget> clientScopeRoots;
  final List<InspectTarget> serverScopeRoots;
  final List<SerializedInspectItemDependency> children;

  SerializedInspectDataItem({
    required this.uri,
    required this.components,
    required this.clientScopeRoots,
    required this.serverScopeRoots,
    required this.children,
  });
}

class SerializedInspectItemDependency {
  final String uri;
  final DirectiveTarget dir;
  final bool onClient;
  final bool onServer;
  final DirectiveTarget? invalidOnClient;
  final DirectiveTarget? invalidOnServer;

  SerializedInspectItemDependency({
    required this.uri,
    required this.dir,
    required this.onClient,
    required this.onServer,
    this.invalidOnClient,
    this.invalidOnServer,
  });
}
