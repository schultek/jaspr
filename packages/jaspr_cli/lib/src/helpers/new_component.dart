...
class NewComponentCommand extends BaseCommand with PubspecHelper, FlutterEmbedSetupHelper {
  NewComponentCommand({super.logger}) {
    argParser.addOption(
      'path',
      abbr: 'p',
      help:
  ...
  void dryRunFlutterViewComponent(Directory dir, String name, Directory projectRoot) {
    ...
    final missingDeps = ['flutter', 'jaspr_flutter_embed'].where((d) => !hasDep(pubspecMap, d));
    final missingDevDeps = [
      'flutter_lints',
      'flutter_test',
    ].where((d) => !hasDep(pubspecMap, d, isDevDependency: true));
