...
''');
  if (!isServe) {
    sb.writeln('''  const modulesNeedingSkwasm = ${modulesNeedingSkwasmJson};''');
  }
  sb.writeln('''
...
''');
  if (isServe) {
    sb.writeln('''  function getWasmImportsDetail(arrayBuffer) {
...
''');
  if (isServe) {
    sb.writeln('''  async function checkAndPrepareImportsForModule(response) {
    ...
    sb.writeln('''  async function checkAndPrepareImportsForModule(moduleName, response) {
  ...
  if (isServe) {
    sb.writeln('''      await checkAndPrepareImportsForModule(response);''');
  } else {
    sb.writeln('''      await checkAndPrepareImportsForModule(m.split('/').pop(), response);''');
  }
  sb.writeln('''      return callback(m, response);
    })();
    moduleLoadingCache.set(m, loadPromise);
    return loadPromise;
  }
...
''');
  if (isServe) {
    sb.writeln('''  const baseBytes = await baseResponse.clone().arrayBuffer();
  ...
  sb.writeln('''''');
  if (isServe) {
    sb.writeln('''  if (needsSkwasmImmediately) {
    const skwasmInstance = await getSkwasmInstance();
    Object.assign(imports, {
      skwasm: skwasmInstance.wasmExports,
      skwasmWrapper: skwasmInstance,
    ...
    if (needsSkwasmImmediately) {
      sb.writeln('''  const skwasmInstance = await getSkwasmInstance();
  Object.assign(imports, {
    skwasm: skwasmInstance.wasmExports,
    skwasmWrapper: skwasmInstance,
    ffi: {
      ...
      sb.writeln('''  imports.skwasmWrapper = {
    addFunction: (...args) => {
      if (!realSkwasmInstance) {
        throw new Error("skwasmWrapper.addFunction called before skwasm was loaded!");
      }
  ...
  if (isServe) {
    ...
    sb.writeln('''      await checkAndPrepareImportsForModule(wasmUri.split('/').pop(), response);''');
  }
  sb.writeln('''      return [response, await mjsModulePromise];
    }
  });
