...
- `jaspr new`: Create a new component or content page inside an existing project.
...
Inside an existing project you can scaffold single files with:

```shell
jaspr new component ContactInfo
jaspr new page getting_started
```

By default `jaspr new component` creates a `StatelessComponent` in `lib/components/`. Use `--stateful`, `--async`, `--inherited` or `--flutter` to pick another component type, and `--client`, `--with-styles` or `--with-test` to extend the generated file.

`jaspr new page` creates a [`jaspr_content`](/content) page in your `content/` directory and fills in the frontmatter from options like `--title`, `--layout` or `--index`.

Both subcommands accept `--path` to choose where the file is created and `--dry-run` to preview the changes without writing anything.
