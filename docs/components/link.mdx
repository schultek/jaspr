...
tag and use the [`Router`s](/components/router) `push()` or `replace()` method to navigate to the target route when clicked.
...
- `target`: The `target` attribute value applied to the anchor element.
- `referrer`: The `referrerpolicy` attribute value applied to the anchor element.
- `classes`, `styles`, `attributes`: Other standard properties applied to the anchor element.
