...
- **Discontinued `jaspr_web_compilers`**: Flutter support can now be enabled through the `--use-flutter-sdk` cli flag with no special dependency.
...
Further, to enable Flutter plugin support pass the `--use-flutter-sdk` flag to the `jaspr serve` and `jaspr build` commands. This is enabled by default when using Flutter embedding.