...
To update to this version, run `dart pub global activate jaspr_cli 0.21.0-dev.1` and set your pubspec dependencies of `jaspr`, `jaspr_builder` and `jaspr_test` to `^0.21.0-dev.1`.
</Info>
...
- **Expanded CSS Support:** New CSS properties like `justifyItems`, `justifySelf`, `alignContent`, `filter`, and `backdropFilter` are now available for more flexible styling.
