...
The css method can be used in a couple of different ways that combined gives you the full styling experience:
...
You should try to use ids or unique class names as selectors to prevent unwanted affects on other components or conflicting style definitions.
</Warning>
...
The default `css()` method takes a selector and a chaining of styles like this:

```dart
css('.main').box(width: 100.px, minHeight: 100.vh).text(color: Colors.black)
```
