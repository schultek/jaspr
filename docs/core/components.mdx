...
As usual for web, styling is done through a combination of CSS attributes, either in a **Stylesheet** or inline though the **`style` attribute** of the parent elements. For example rendering a simple heading would look like this:
...
- can return multiple components inside the `build()` method, just like the [`StatelessComponent`](#3️⃣-statelesscomponent)

In every other aspect, this component behaves the same as Flutters `StatefulWidget`.

### 5️⃣ InheritedComponent
