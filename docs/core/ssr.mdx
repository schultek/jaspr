...
You can either specify the same entrypoint (usually `main.dart`) for both targets, or use separate
entrypoints to e.g. configure some platform specific state or services.
...
2. The entrypoint on the server will:
    ...
    - default to `lib/main.dart` if the file exists or
    - fallback to `web/main.dart`
