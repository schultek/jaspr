...
1. Passing [style rules](/utils/at_css#defining-style-rules) to the [Document](/components/document)`.styles` parameter.
   This will render the provided styles into a global stylesheet inside the `<head>` element.

2. Using the [@css](/utils/at_css#registering-style-rules) annotation to automatically render the annotated styles into a global stylesheet inside
   the `<head>` element.
...
When you are in **static** or **server** mode, add this to the [Document](/components/document)s `head` parameter:
...
You can use the [Document.head](/components/document#document-head) component to dynamically set a value in the `<head>`
from any component.
...
and include the generated output `.css` file as an [external stylesheet](#external-stylesheets):
