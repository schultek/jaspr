...
Read more about this approach in the documentation for [@çss](/utils/at_css).
...
1. Passing [Style Rules](/utils/at_css#defining-style-rules) to [Document(styles: ...)](/components/document#parameters). This will render the provided styles into a global stylesheet inside the `<head>` element.

2. Using the [@css](/utils/at_css#registering-style-rules) annotation on a global variable to automatically render the annotated styles into a global stylesheet inside the `<head>` element.
