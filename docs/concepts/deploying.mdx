...
If your project is depending on either `flutter` for element embedding or uses the `--use-flutter-sdk` flag you need to 
modify the created workflow file to also pull in flutter, otherwise the build will result in an error.
...
If your project is depending on either `flutter` for element embedding or uses the `--use-flutter-sdk` flag you need to
modify the `Dockerfile` to also have flutter installed in the build image, otherwise the build will result in an error.
...
No additional configuration is needed. Globe's build pipeline automatically detects and sets up projects that use Flutter for element embedding.