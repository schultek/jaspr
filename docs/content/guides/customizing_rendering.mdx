...
You can create your own layout by implementing the `PageLayout` interface:

```dart
class BlogLayout implements PageLayout {
  @override
  Component build(PageContext context, Component content) {
    return DomComponent(
      tag: 'div',
      classes: ['blog-layout'],
      children: [
        Header(title: context.page.data['title'] ?? 'Blog'),
        DomComponent(
          tag: 'main',
          children: [content],
        ),
        Footer(
          ...
          links: [
            Link(text: 'Home', href: '/'),
            Link(text: 'Archive', href: '/archive'),
...
You can specify which layout to use for a specific page in its frontmatter:
...
This will use the blog layout instead of the default one.
```

For this to work, you need to register your layout with a name:

```dart
ContentApp(
  layouts: [
    NamedLayout('blog', BlogLayout()),
    NamedLayout('docs', DocsLayout()),
  ],
)
```
