...
By default, `jaspr_content` loads page data (frontmatter, data from `DataLoader`s) lazily, i.e., only when a page is requested. This is efficient for development and large sites in server mode.

However, sometimes you might need all page data to be available upfront, for example, to generate a sitemap, an index page listing all blog posts, or if one page's content depends on data from other pages. This is where eager loading comes in.
...
- All pages' data (as defined by `DataLoader`s and frontmatter) is loaded at application startup.
- The full list of `Page` objects is available via `PageContext.pages` within your components, allowing you to iterate over all site pages.

Page rendering (parsing content, applying extensions, running template engines) is still done on-demand when a page is requested, even in eager mode.

## Custom Route Builder

`jaspr_content` automatically creates a `Router` component with all the routes discovered by your `RouteLoader`s. If you need to add custom routes alongside your content pages (e.g., a custom landing page, API endpoints handled by Jaspr) or want to wrap the router in other components, you can use the `routerBuilder` parameter in `ContentApp.custom`.
...
import 'package:jaspr_router/jaspr_router.dart' hide RouteLoader; // Avoid name clash

class HomePage extends StatelessComponent {
  @override
  Iterable<Component> build(BuildContext context) sync* {
    yield DomComponent(tag: 'h1', child: Text('Welcome to My Custom Site!'));
  }
}
...
void main() {
  ...
  runApp(
    ContentApp.custom(
      ...
      routerBuilder: (List<List<RouteBase>> contentRoutes) {
        return Router(
          routes: [
            ...
            ...contentRoutes.expand((r) => r).toList(),
          ],
        );
      },
    ),
