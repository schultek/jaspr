...
class TestAsyncUtils {
  ...
  static Future<T> guard<T>(Future<T> Function() body) {
    ...
    Future<T> completionHandler(dynamic error, StackTrace? stack) {
      ...
      while (_scopeStack.isNotEmpty) {
        ...
        if (!leaked) {
          information.add('Asynchronous call to guarded function leaked.');
          information.add('You must use "await" with all Future-returning test APIs.');
          leaked = true;
        }
        ...
        if (originalGuarder != null) {
          information.add('The test API method "${originalGuarder.methodName}" '
      ...
      if (leaked) {
        if (error != null) {
          information
              .add('An uncaught exception may have caused the guarded function leak. The exception was:\n$error');
          information.add('The stack trace associated with this exception was:\n$stack');
        }
        throw information.join('\n');
      }
      if (error != null) return Future<T>.error(error! as Object, stack);
  ...
  static void guardSync() {
    ...
    if (originalGuarder != null && collidingGuarder != null) {
      ...
      if (originalGuarder.className == null) {
        ...
        information.add('The guarded "${originalGuarder.methodName}" function '
            'was called from ${originalGuarder.callerFile} '
            'on line ${originalGuarder.callerLine}.');
      } else {
        originalName = '$originalKind (${originalGuarder.className}.${originalGuarder.methodName})';
        information.add('The guarded method "${originalGuarder.methodName}" '
            'from class ${originalGuarder.className} '
            'was called from ${originalGuarder.callerFile} '
            'on line ${originalGuarder.callerLine}.');
      }
      ...
      if ((originalGuarder.className == collidingGuarder.className) &&
        ...
        information.add('Then, the "${collidingGuarder.methodName}" function '
            'was called ${again}from ${collidingGuarder.callerFile} '
            'on line ${collidingGuarder.callerLine}.');
      } else {
        collidingName = '$collidingKind (${collidingGuarder.className}.${collidingGuarder.methodName})';
        information.add('Then, the "${collidingGuarder.methodName}" method '
      ...
      information.add('The first $originalName '
      ...
      if (collidingGuarder.className == null && collidingGuarder.methodName == 'expect') {
        information.add('If you are confident that all test APIs are being called using "await", and '
  ...
  static void verifyAllScopesClosed() {
    if (_scopeStack.isNotEmpty) {
      ...
      for (final _AsyncScope scope in _scopeStack) {
        ...
        if (guarder != null) {
          information.add('The guarded method "${guarder.methodName}" '
  ...
  static _StackEntry? _findResponsibleMethod(StackTrace rawStack, String method, List<String> information) {
    ...
    if (index < stack.length) {
      ...
      information
          .add('(Unable to find the method that called $_className.$method(). The stack may be incomplete or bogus.)');
    }
    return null;
  }
}
